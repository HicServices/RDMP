...
namespace CatalogueManager.TestsAndSetup.StartupUI
{
    ...
    /// 
    /// Tier 3 - Plugin databases, these are wild and uncontrollable by RDMP.  All functionality to interact with these is stores in the plugin that created them.  For example you
    /// might decide that you wanted to do your own unique anonymisation method and create a plugin which uses its own database schema to store/generate anonymous identifiers.
    /// 
    /// This control indicates that the RDMP startup process has found a plugin which has tier 3 databases associated with it.  You will see any/all tier 3 databases found by the 
    /// plugin in StartupUIMainForm under tier 3 (See ManagedDatabaseUI).
    /// </summary>
