...
/// <summary>
///     Base class for all tests that want to create objects only in memory (and not in database like
///     <see cref="DatabaseTests" />)
/// </summary>
[Category("Unit")]
public class UnitTests
{
    /// <summary>
    ///     The number of seconds that have to differ between two DateTime objects in method <see cref="AreAboutTheSameTime" />
    ///     before
    ...
    protected HashSet<string> SkipTheseTypes = new(new[]
    {
        "TestColumn",
        "ExtractableCohort",
        "DQEGraphAnnotation",
    ...
    protected IRDMPPlatformRepositoryServiceLocator RepositoryLocator { get; }
    ...
    {
        return new ConsoleInputManager(locator ?? RepositoryLocator, new ThrowImmediatelyCheckNotifier())
            { DisallowInput = true };
    }
    ...
    public static T WhenIHaveA<T>(MemoryDataExportRepository repository) where T : DatabaseEntity
    {
        ...
        {
            var sds = WhenIHaveA<SelectedDataSets>(repository);
            var container = new FilterContainer(repository);
        ...
        if (typeof(T) == typeof(DeployedExtractionFilterParameter))
        {
            ...
            return (T)filter.GetFilterFactory().CreateNewParameter(filter, "DECLARE @had as varchar(100)");
        }
    ...
    /// <summary>
    ///     Asserts that the two objects are basically the same except for IDs/Repositories.  This includes checking all public
    ///     properties
    ///     that are not in the <see cref="IgnorePropertiesWhenDiffing" /> list.  Date fields will be validated as equal if
    ///     they are within
    ...
    /// <summary>
    ///     Returns a properly initialized object of Type <paramref name="t" /> which must be a <see cref="DatabaseEntity" />
    ///     that
