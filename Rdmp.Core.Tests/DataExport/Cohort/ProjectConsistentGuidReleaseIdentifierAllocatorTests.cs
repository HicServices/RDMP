...
internal class ProjectConsistentGuidReleaseIdentifierAllocatorTests : DatabaseTests
{
    ...
    public void TestPreserveHistoricalReleaseIdentifiers(DatabaseType databaseType)
    {
        ...
        var privateIdentifierDataType = db.Server.GetQuerySyntaxHelper().TypeTranslater
        ...
        var ect = wizard.CreateDatabase(new PrivateIdentifierPrototype("chi", privateIdentifierDataType),
        ...
        var req = new CohortCreationRequest(p, new CohortDefinition(null, "TestCohort1", 1, p.ProjectNumber.Value, ect),
            DataExportRepository, "Ignoreme");
        ...
        {
            con.Open();
            db.Server.GetCommand($"UPDATE {cohortTable} SET ReleaseId='0x0127' WHERE ReleaseId='0x0128'", con)
                .ExecuteScalar();
        }
