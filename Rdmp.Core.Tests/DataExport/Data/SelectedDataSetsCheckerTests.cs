...
public class SelectedDataSetsCheckerTests : TestsRequiringAnExtractionConfiguration
{
    ...
    public void TestExtractionProgress_MidwayWithNoAuditRecord()
    {
        ...
        Assert.That(
            ex.Message, Is.EqualTo("R0016 ExtractionProgress 'mybatch' is 'in progress' (ProgressDate is not null) but there is no audit of previously extracted SQL (needed for checking cohort changes)"));
    }
    ...
    public void TestExtractionProgress_AuditRecordHasDifferentCohort()
    {
        ...
        var ex = Assert.Throws<Exception>(() => checker.Check(ThrowImmediatelyCheckNotifier.Quiet));
        Assert.That(
            ex.Message, Is.EqualTo($"R0017 ExtractionProgress 'mybatch' is 'in progress' (ProgressDate is not null) but we did not find the expected Cohort WHERE Sql in the audit of SQL extracted with the last batch.  Did you change the cohort without resetting the ProgressDate? The SQL we expected to find was '[{TestDatabaseNames.Prefix}CohortDatabase]..[Cohort].[cohortDefinition_id]=-599'"));
