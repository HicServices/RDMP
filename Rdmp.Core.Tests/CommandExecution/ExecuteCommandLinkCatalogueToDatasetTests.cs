...
internal class ExecuteCommandLinkCatalogueToDatasetTests : CommandCliTests
{
    ...
    public void TestLinkCatalogueToDataset()
    {
        ...
        _t1.SaveToDatabase();
        _t2.SaveToDatabase();
        var _c1 = new ColumnInfo(GetMockActivator().RepositoryLocator.CatalogueRepository, "test.db", "varchar(10)",
            _t1);
        var _c2 = new ColumnInfo(GetMockActivator().RepositoryLocator.CatalogueRepository, "test.db", "int", _t2);
        _c1.SaveToDatabase();
        _c2.SaveToDatabase();
        var _ci1 = new CatalogueItem(GetMockActivator().RepositoryLocator.CatalogueRepository, _cata1,
            "PrivateIdentifierA");
        _ci1.SetColumnInfo(_c1);
        var _ci2 = new CatalogueItem(GetMockActivator().RepositoryLocator.CatalogueRepository, _cata2,
            "PrivateIdentifierB");
        ...
        var founddataset = GetMockActivator().RepositoryLocator.CatalogueRepository
            .GetAllObjects<Core.Curation.Data.Dataset>().First();
        var foundCatalogue = GetMockActivator().RepositoryLocator.CatalogueRepository.GetAllObjects<Catalogue>()
            .First(static c => c.Name == "Dataset1");
        ...
        foreach (var ci in columInfo) Assert.That(ci.Dataset_ID, Is.EqualTo(founddataset.ID));
    ...
    public void TestLinkCatalogueToDatasetNotAll()
    {
        ...
        var founddataset = GetMockActivator().RepositoryLocator.CatalogueRepository
            .GetAllObjects<Core.Curation.Data.Dataset>().First();
        var foundCatalogue = GetMockActivator().RepositoryLocator.CatalogueRepository.GetAllObjects<Catalogue>()
            .First(c => c.Name == "Dataset1");
        var linkCmd = new ExecuteCommandLinkCatalogueToDataset(GetMockActivator(), foundCatalogue, founddataset, false);
        Assert.DoesNotThrow(linkCmd.Execute);
        var columInfo = GetMockActivator().RepositoryLocator.CatalogueRepository.GetAllObjects<CatalogueItem>()
            .Where(ci => _cata1.CatalogueItems.Contains(ci));
        foreach (var ci in columInfo) Assert.That(ci.ColumnInfo.Dataset_ID, Is.EqualTo(founddataset.ID));

        var columInfo2 = GetMockActivator().RepositoryLocator.CatalogueRepository.GetAllObjects<CatalogueItem>()
            .Where(ci => _cata2.CatalogueItems.Contains(ci));
        foreach (var ci in columInfo2) Assert.That(ci.ColumnInfo.Dataset_ID, Is.Null);
    }
    ...
    public void TestLinkCatalogueToDatasetBadDataset()
    {
        var cmd = new ExecuteCommandCreateDataset(GetMockActivator(), "dataset");
        Assert.DoesNotThrow(cmd.Execute);
        GetMockActivator().RepositoryLocator.CatalogueRepository.GetAllObjects<Core.Curation.Data.Dataset>().First();
        var linkCmd = new ExecuteCommandLinkCatalogueToDataset(GetMockActivator(),
            new Catalogue(GetMockActivator().RepositoryLocator.CatalogueRepository, "catalogue"), null, false);
        Assert.Throws<ImpossibleCommandException>(linkCmd.Execute);
    }
