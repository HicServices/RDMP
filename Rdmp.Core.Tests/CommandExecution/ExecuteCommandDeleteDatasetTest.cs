...
internal class ExecuteCommandDeleteDatasetTest: CommandCliTests
{
    ...
    public void TestDeleteExistingDataset()
    {
        ...
        Assert.That(GetMockActivator().RepositoryLocator.CatalogueDbContext.GetAllObjects<Rdmp.Core.Curation.Data.Dataset>(), Has.Length.EqualTo(1));
        var founddataset = GetMockActivator().RepositoryLocator.CatalogueDbContext.GetAllObjects<Core.Curation.Data.Dataset>().First(static ds => ds.Name == "dataset");
        var delCmd = new ExecuteCommandDeleteDataset(GetMockActivator(), founddataset);
        Assert.DoesNotThrow(() => delCmd.Execute());
        Assert.That(GetMockActivator().RepositoryLocator.CatalogueDbContext.GetAllObjects<Rdmp.Core.Curation.Data.Dataset>(), Is.Empty);
    }
