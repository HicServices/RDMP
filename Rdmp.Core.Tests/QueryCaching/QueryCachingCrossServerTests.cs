...
internal class QueryCachingCrossServerTests : TestsRequiringA
{
    ...
    /// <summary>
    ///     Tests running a simple cic with a single UNION container holding an aggregate.  The aggregate optionally has a
    ///     parameter on it.  The
    ...
    /// <summary>
    ///     Tests running a simple cic with a single UNION container holding two aggregates.  The aggregates both have their
    ///     own parameter
    ///     called @date_of_max but they have different values.  This means the SQL generated needs to be adjusted so they
    ///     don't collide /
    ...
    /// <summary>
    ///     Tests running a cic in which there are 2 aggregates both hospitalizations with 1 EXCEPT the other.  The expected
    ///     result is 0
    ///     patients
    /// </summary>
    /// <param name="dbType"></param>
    ...
    /// <summary>
    ///     Tests the ability to run a cic in which there are 2 tables, one patient index table with the dates of biochemistry
    ///     test codes
    ///     and another which joins and filters on the results of that table to produce a count of patients hospitalised after
    ///     an NA test.
    ...
    ///     A patient index table on server 1 and a table on server 2 to which it must be joined.
    ///     <para>
    ///         This should fail because the only way to combine the two datasets is with a join and they are on separate
    ///         servers
    ...
    /// <summary>
    ///     Creates a table HospitalAdmissions that uses the patient index table <paramref name="joinable" /> to return
    ///     distinct patients
    ...
    private static void AssertNoErrors(CohortCompiler compiler)
    {
        ...
        foreach (var kvp in compiler.Tasks.Keys)
            TestContext.WriteLine(
                $"{i++} - {kvp} | {kvp.GetType()} | {kvp.State} | {kvp.CrashMessage} | {kvp.FinalRowCount} | {kvp.GetCachedQueryUseCount()}");
    ...
    /// <summary>
    ///     Asserts that the given <paramref name="task" /> (when run on its own) completed successfully and that the SQL
    ///     executed
    ...
    /// <summary>
    ///     Asserts that the given <paramref name="container" /> ran successfully and fetched its results from the cache (if
    ///     any) with the
