...
namespace Rdmp.Core.Tests.Curation
{
    class ProjectSpecificCatalogueTests : DatabaseTests
    {
        ...
        public void SetupTests(int projectCount = 0, int projectSpecificCount = 0, int extractionCount = 0)
        {
            ...
            _eds1 = _activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).FirstOrDefault() ?? new ExtractableDataSet(_activator.RepositoryLocator.DataExportRepository, _catalogue);
            ...
            if (projectSpecificCount > 0)
            {
                ...
                _eds1 = _activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).FirstOrDefault();
            }
            ...
            if (extractionCount > 0)
            {
                ...
                Assert.DoesNotThrow(() => ec.AddDatasetToConfiguration(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).First()));
                ec.SaveToDatabase();
            }
        ...
        public void MakeCatalogueNotProjectSpecificWithTwoProjectsTest()
        {
            SetupTests(2, 2, 0);
            MakeNotProjectSpecific(_catalogue, _eds1, _project1);
            MakeNotProjectSpecific(_catalogue, _activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWithParent<ExtractableDataSet>(_catalogue).FirstOrDefault(eds => eds.Projects.Select(p => p.ID).Contains(_project2.ID)),_project2);
        }
        ...
        public void MakeSingleCatalogueNotProjectSpecificWithTwoProjectsTest()
        {
            ...
            Assert.That(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).Where(eds => eds.Projects.Any()).Count, Is.EqualTo(1));
        }
        ...
        {
            SetupTests(2, 2, 1);
            MakeNotProjectSpecific(_catalogue, _activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWithParent<ExtractableDataSet>(_catalogue).FirstOrDefault(eds => eds.Projects.Contains(_project2)),_project1);
            Assert.That(_activator.RepositoryLocator.CatalogueDbContext.GetObjectByID<Catalogue>(_catalogue.ID).IsProjectSpecific(_activator.RepositoryLocator.DataExportRepository), Is.True);
            Assert.That(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).Where(eds => eds.Projects.Any()).Count, Is.EqualTo(1));
        }
        ...
        public void MakeSingleCatalogueNotProjectSpecificWithTwoProjectsAndOneExtractionTest()
        {
            ...
            Assert.That(_activator.RepositoryLocator.CatalogueDbContext.GetObjectByID<Catalogue>(_catalogue.ID).IsProjectSpecific(_activator.RepositoryLocator.DataExportRepository), Is.True);
            Assert.That(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).First().Projects.Count, Is.EqualTo(2));
        }
        ...
        {
            SetupTests(2, 0, 1);
            Assert.That(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).Count, Is.EqualTo(1));
            Assert.That(_activator.RepositoryLocator.DataExportCatalogueDbContext.GetAllObjectsWhere<ExtractableDataSet>("Catalogue_ID", _catalogue.ID).First().Projects.Any(), Is.False);
