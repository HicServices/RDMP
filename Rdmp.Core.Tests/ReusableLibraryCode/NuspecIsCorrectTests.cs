...
namespace Rdmp.Core.Tests.ReusableLibraryCode
{
    ...
    class NuspecIsCorrectTests
    {
        ...
        private static readonly Regex RPackageRef = new Regex(@"<PackageReference\s+Include=""(.*)""\s+Version=""([^""]*)""", RegexOptions.IgnoreCase|RegexOptions.Compiled|RegexOptions.CultureInvariant);

        //<dependency id="CsvHelper" version="12.1.2" />
        private static readonly Regex RDependencyRef = new Regex(@"<dependency\s+id=""(.*)""\s+version=""([^""]*)""", RegexOptions.IgnoreCase|RegexOptions.Compiled|RegexOptions.CultureInvariant);
        ...
        public void TestDependencyCorrect( string csproj, string nuspec, string packagesMarkdown)
        {
            ...
            foreach (Match p in RPackageRef.Matches(File.ReadAllText(csproj)))
            {
                ...
                if (!Analyzers.Contains(package) && nuspecContent != null)
                {
                    ...
                    foreach (Match d in RDependencyRef.Matches(nuspecContent))
                    {
