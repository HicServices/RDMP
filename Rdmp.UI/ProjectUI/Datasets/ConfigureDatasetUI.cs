...
/// <summary>
///     Allows you to choose which columns you want to extract from a given dataset (Catalogue) for a specific research
///     project extraction (ExtractionConfiguration).  For example
///     Researcher A wants prescribing dataset including all the Core columns but he also has obtained governance approval
///     to receive Supplemental column 'PrescribingGP' so the
///     configuration would need to include this column.
///     <para>
///         On the left you can see all the available columns and transforms in the selected dataset (see
///         ExtractionConfigurationUI for selecting datasets).  You can add these by selecting them
///         and pressing the '>' button.  On the right the QueryBuilder will show you what columns are currently included
///         in the researchers extract.
///     </para>
///     <para>
///         Depending on which columns you have selected the QueryBuilder may be unable to generate a query (for example if
///         you do not add the IsExtractionIdentifier column - See
...
public partial class ConfigureDatasetUI : ConfigureDatasetUI_Design, ILifetimeSubscriber
{
    ...
    public ConfigureDatasetUI()
    {
        ...
        var tableInfoIcon = Image.Load<Rgba32>(CatalogueIcons.TableInfo).ImageToBitmap();
    ...
    /// <summary>
    ///     The left list contains ExtractionInformation from the Data Catalogue, this is columns in the database which could
    ///     be extracted
    ///     The right list contains ExtractableColumn which is a more advanced class that contains runtime configurations such
    ///     as order to be outputed in etc.
    /// </summary>
    ...
    /// <summary>
    ///     Determines whether this potential extractable column (identified by the catalogue) is already selected and
    ///     configured
    /// </summary>
    ...
    /// <summary>
    ///     Removes all currently selected <see cref="ExtractableColumn" /> from the <see cref="SelectedDataSets" /> (leaving
    ///     it empty)
    /// </summary>
