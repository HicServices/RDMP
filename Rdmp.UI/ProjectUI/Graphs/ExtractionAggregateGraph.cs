...
/// <summary>
///     As part of the ongoing effort to ensure extracted project data is correct (releasing incorrect data to a researcher
///     is about the worst mistake you can make as a data analyst) a
///     feature was added which allows execution of aggregate graphs (See AggregateGraph) on the project extraction SQL.
///     <para>
///         This control lets you execute a dataset aggregate graph but restricted to the dataset extraction configuration
///         you are working on (only including the records which will be extracted
///         when you execute the project extraction).  This is achieved by adjusting the WHERE statement in the Aggregate
///         SQL built by the AggregateBuilder to include the Cohort joins and any
///         data extraction filters.  Because of this transformation, depending on your indexes the graphs may take a long
///         time to run (especially if the basic dataset aggregate graph takes a
///         long time to execute).
///     </para>
///     <para>
///         You can speed up these graphs by viewing the SQL generated by the system and running it in SQL Management
///         Studio with the Query Analyser turned on and creating appropriate indexes.
///     </para>
/// </summary>
