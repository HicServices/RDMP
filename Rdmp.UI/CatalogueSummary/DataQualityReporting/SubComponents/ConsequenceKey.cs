...
/// <summary>
///     All validation rules configured in the RDMP have an associated 'Consequence', this is like a severity level.  The
///     lowest is 'Missing' and indicates that the failure of the
///     validation rule means that an expected value is not present.  The worst is 'Invalidates Row' which indicates that
///     the validation failure is so serious that the entire row
///     is useless (e.g. a hospital admissions record with no patient identifier making it unlinkable).  See
///     SecondaryConstraintUI for more information on how validation rules are
///     interpreted.
