...
/// <summary>
///     Part of DatasetRaceway, this control shows a miniature bar chart with 1 bar per month of data held within the
///     dataset.  Bar colour indicates the proportion of records in that month
///     which are passing/failing validation (green = passing, red = failing).  The month axis is shared across all
///     datasets in the DatasetRaceway meaning that you see a continuous axis
///     that spans the whole length of time you have been holding data for in any of your datasets.  The data for this
///     graph comes from the Data Quality Engine evaluations database so if
///     you have never run the DQE on a given dataset it will not appear in the DatasetRaceway.
///     <para>
///         In DatasetRaceway you can adjust the scope of the axis down from 'All Time' to 'Last Decade',  'Last Year' or
///         'Last 6 months' if you just want to see how up-to-date each dataset
///         is in finer detail.
///     </para>
///     <para>
///         The overall effect of this control is to allow you to rapidly identify any datasets that you host which have
///         suddenly started failing validation, see where each dataset starts and
...
public partial class RacewayRenderAreaUI : UserControl, INotifyMeOfEditState
{
    ...
    private readonly Timer _mouseHeldDownTimer = new();
    ...
    private readonly Pen _verticalLinesPen = new(Color.FromArgb(150, Color.White));

    private long frameLimiter;

    private readonly object oPeriodicityDictionaryLock = new();
    private readonly Dictionary<Rectangle, Catalogue> rectDeleteButtons = new();

    private readonly Dictionary<Rectangle, Catalogue> rectNoDQE = new();
        ...
        float eachRaceLaneHasThisMuchYSpace, double middleLineOfCatalogueLabelY)
    {
        var redGradientBrush = new LinearGradientBrush(
            ...
            new Point(Width, (int)eachRaceLaneHasThisMuchYSpace)
