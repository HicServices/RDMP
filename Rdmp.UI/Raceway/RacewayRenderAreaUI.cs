...
public partial class RacewayRenderAreaUI : UserControl,INotifyMeOfEditState
{
    ...
    private void _mouseHeldDownTimer_Tick(object sender, EventArgs e)
    {
        ...
        if (_currentScrollAction == ScrollActionUnderway.None || DateTime.Now.Subtract(_currentScrollActionBegan).TotalMilliseconds < 200)
            return;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        lock (oPeriodicityDictionaryLock)
        {
            ...
            foreach (var kvp in _periodicityDictionary.OrderBy(kvp => kvp.Value == null ? $"zzzzzz{kvp.Key.Name}" : kvp.Key.Name))
            {
                ...
                else
                {
                    ...
                    e.Graphics.DrawString(catalogue.Name, Font, Brushes.White, new Point(0, (int)middleLineOfCatalogueLabelY));
                }
        ...
        if (_allowScrollDown && _allowScrollUp && GetMaxScrollDownToIndex() != 0)
        {
            ...
            e.Graphics.DrawLine(Pens.Black, Width - 20, yHeightOfScrollIndicator+1, Width - 1, yHeightOfScrollIndicator+1);
        }
    }
