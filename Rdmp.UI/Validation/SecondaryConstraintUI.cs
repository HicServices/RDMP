...
/// <summary>
///     Part of <see cref="ValidationSetupUI" />, this control is for configuring/viewing a single validation rule on a
///     column/transform of a dataset (Catalogue).  For example it might be a NotNull
///     validation constraint which means rows with a null value in this field will fail validation.  Why would you do this
///     you ask? when you can have a database constraint in your data
///     repository that prevents null values? well sometimes research data is quite dirty and if a problematic
///     field(especially if it is a non-essential column) is sometimes worth allowing
///     it through even though it's null and highlighting problem records with the validation rule NotNull.
///     <para>
///         Other secondary constraints include Regex patterns, standard regexes (See StandardRegexUI), referential
///         integrity constraints etc.
///     </para>
///     <para>
///         Each constraint has a Consequence (Missing, Wrong, Invalidates Row) these are used to classify the state of
///         each row in the Data Quality Engine when running validation.  For example
///         if you have 2 cells in a row that are both failing validation, one with a consequence of Missing and one with a
///         consequence of Wrong then the entire row is classified as 'Wrong'
///         overall.
///     </para>
/// </summary>
public partial class SecondaryConstraintUI : UserControl
{
    ...
    /// </summary>
    private readonly PropertyInfo[] _requiredProperties;


    private readonly bool loadingComplete;

    /// <summary>
    ///     this UI exists to modify this property, the secondary constraint, it is entirely driven by reflection so should
    ///     handle any SecondaryConstraint you throw at it
    /// </summary>
