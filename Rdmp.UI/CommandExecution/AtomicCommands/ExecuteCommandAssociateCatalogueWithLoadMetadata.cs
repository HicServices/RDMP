                    ...
                    if (tasks.Length == 1)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(task) && !task.Equals(cata.LoggingDataTask))
                        {
                            ...
                            if (liveServers.Count() <= 1)
                            {
                                ...
                                || MessageBox.Show("Do you want to set Catalogue '" + cata.Name + "' to use shared logging task '" + task + "' instead of it's current Logging Task '" + cata.LoggingDataTask + "' (All Catalogues in a load must share the same task and logging servers)?", "Synchronise Logging Tasks", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                {
