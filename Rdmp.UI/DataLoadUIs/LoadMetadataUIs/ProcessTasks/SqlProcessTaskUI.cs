...
/// <summary>
///     Lets you view/edit a single SQL file execution load task.  This SQL script will be run at the appropriate time in
///     the data load (depending on which stage it is at and the order in the
///     stage.  The SQL will be executed on the database/server that corresponds to the stage.  So an Adjust RAW script
///     cannot modify STAGING since those tables won't even exist at the time of
///     execution and might even be on a different server.
///     <para>
///         You should avoid modifying Live tables directly with SQL since it circumvents the 'no duplication',
///         'RAW->STAGING->LIVE super transaction' model of RDMP.
///     </para>
/// </summary>
