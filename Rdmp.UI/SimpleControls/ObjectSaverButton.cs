...
/// <summary>
///     Allows saving of any DatabaseEntity.  When public properties are changed on the object the control automatically
///     lights up and tracks the changes.  Clicking
///     the Undo button will reset the DatabaseEntity to the state before the changes were made / when it was last saved.
///     Undo is a toggle from last saved state to
///     current user edit state and back again (i.e. not a tracked history).  In order to use an object saver button you
///     should add it to an RDMPSingleDatabaseObjectControl
///     and call SetupFor on the DatabaseObject.  You should also mark your control as ISaveableUI and implement the single
///     method on that interface so that shortcuts
...
public class ObjectSaverButton
{
    ...
    private readonly Bitmap _redoImage;
    private bool _undo = true;
    private readonly Bitmap _undoImage;

    private RevertableObjectReport _undoneChanges;

    private readonly ToolStripButton btnSave = new("Save", FamFamFamIcons.disk.ImageToBitmap());
    private readonly ToolStripButton btnUndoRedo = new("Undo", FamFamFamIcons.Undo.ImageToBitmap());
    ...
    /// <summary>
    ///     Function to carry out some kind of proceedure before the object is saved.  Return true if you want the save to
    ///     carry on and be applied or false to abandon the save attempt.
    /// </summary>
