...
/// <summary>
///     There are two main event systems in play in the RDMP.  There is Checking and Progress.  Checking activities are
///     tasks that should be supervised and can block asking the user
///     whether or not a proposed fix to a problem should be applied (See ChecksUI).  Progress activities are messages only
///     and can also include numerical update messages indicating
///     that progress is made towards a fixed number e.g. you could get 1000 messages over the course of an hour reporting
///     how close towards a goal of 1,000,000 records a given task is.
///     <para>This control handles progress messages.  For Checks event system see ChecksUI.</para>
///     <para>
///         ProgressUI handles progress messages of numerical progress (in either records or kilobytes) by updating the
///         datagrid.  Messages appear in the Notifications area and
///         function very similarly to ChecksUI (you can double click them to view the message/copy it / view stack traces
///         etc).  Because classes can be quite enthusiastic about notifying
///         progress this control buffers all messages it receives and only updates the user interface once every 3s (this
///         improves performance).  All date/times come from the buffered messages
...
public partial class ProgressUI : UserControl, IDataLoadEventListener
{
    ...
    /// <summary>
    ///     See HandleFloodOfMessagesFromJob, basically if the message in a progress event changes over time we don't want to
    ///     spam the datagrid so instead we just note that there is a
    ...
    private readonly Bitmap _fail;
    private readonly Bitmap _failEx;

    private readonly Bitmap _information;

    private readonly int _processingTimeColIndex;
    private readonly Bitmap _warning;
    private readonly Bitmap _warningEx;
    private readonly DataTable progress = new();
