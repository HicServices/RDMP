...
/// <summary>
///     Displays detailed breakdown of database queries sent by the RDMP during Performance Logging (See
///     PerformanceCounterUI).  The colour of the row indicates the number of times a database
///     query was sent from that point in the call stack.  Note that this is the number of calls not the time taken to
///     execute the call so you could see poor performance in UI interaction and
...
{
    private static readonly Color Heavy = Color.Red;
    private static readonly Color Light = Color.LawnGreen;
    private ComprehensiveQueryPerformanceCounter _performanceCounter;

    private int _worstOffenderCount;
    private readonly bool collapseToMethod = false;
