...
/// <summary>
///     Many researchers like flat tables they can load into SPSS or STATA or Excel and thus would prefer not to deal with
///     multiple tables if possible.  Storing datasets as flat
///     tables however is often suboptimal in terms of performance and storage space.  Therefore it is possible to
///     configure a dataset (Catalogue) which includes columns from
///     multiple tables.  For example if you have a Header and Results table in which Header tells you when a test was done
///     and by whom including sample volume etc and each test
///     gives multiple results (white blood cell count, red blood cell count etc) then you will obviously want to store it
///     as two separate tables.
///     <para>
///         Configuring join logic in RDMP enables the QueryBuilder to write an SQL SELECT query including a LEFT JOIN /
///         RIGHT JOIN / INNER JOIN over two or more tables.  If you don't
///         understand what an SQL Join is then you should research this before using this window.  Unlike a Lookup (See
///         LookupConfiguration) it doesn't really matter which table contains
///         the ForeignKey and which contains the PrimaryKey since changing the Join direction effectively inverts this
///         logic anyway (i.e. Header RIGHT JOIN Results is the same as Results
///         LEFT JOIN Header).  Configuring join logic in the RDMP database does not affect your data repository in any way
///         (i.e. it doesn't mean we will suddenly start putting referential
///         integrity constraints into your database!).
///     </para>
///     <para>
///         You might wonder why you have to configure JoinInfo information into RDMP when it is possibly already
///         implemented in your data model (e.g. with foreign key constraints).  The
///         explicit record in the RDMP database allows you to hold corrupt/unlinkable data (which would violate a foreign
///         key constraint) and still know that the tables must be joined.
///         Additionally it lets you configure joins between tables in different databases and to specify an explicit
///         direction (LEFT / RIGHT / INNER) which is always the same when it comes
///         time to extract your data for researchers.
///     </para>
///     <para>
///         If you need to join on more than 1 column then just create a JoinInfo for each pair of columns (making sure the
///         direction - LEFT/RIGHT/INNER matches).  For example if the join is
///         Header.LabNumber = Results.LabNumber AND Header.Hospital = Results.Hospital (because of crossover in LabNumber
///         between hospitals) then you would configure a JoinInfo for
...
public partial class JoinConfigurationUI : JoinConfiguration_Design
{
    ...
    private void UpdateValidityAssesment(bool actuallyDoIt = false)
    {
        ...
        try
        {
            var fks = new[] { fk1.SelectedColumn, fk2.SelectedColumn, fk3.SelectedColumn }.Where(f => f != null)
                .ToArray();
            var pks = new[] { pk1.SelectedColumn, pk2.SelectedColumn, pk3.SelectedColumn }.Where(p => p != null)
                .ToArray();
