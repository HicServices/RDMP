...
public sealed partial class NewfindUI : Form
{
    private readonly IActivateItems _activator;
    private Dictionary<IMapsDirectlyToDatabaseTable, DescendancyList> _items;
    private readonly HashSet<Type> _showOnlyTypes = new();
    ...
    private readonly bool _showReplaceOptions;

    private void SimulateClickForAutoFilter<TFilter>()
    {
        var item = newFindToolStrip.Items.Find(typeof(TFilter).Name, false).FirstOrDefault();
        item?.PerformClick();
    }
    ...
    private void PresetFiltersBasedOnFocusItem(RDMPUserControl focusItem)
    {
        ...
        foreach (var t in types.Select(static (value, i) => new { i, value }).Where(t => focusItemType.BaseType?.BaseType?.GenericTypeArguments.FirstOrDefault() == t.value))
        {
            switch (t.i)
            {
                case 0:
    ...
    public NewfindUI(IActivateItems activator, bool showReplaceOptions = true, RDMPUserControl? focusItem = null)
    {
        ...
        olvID.AspectGetter = static m => (m as IMapsDirectlyToDatabaseTable)?.ID;
        olvName.AspectGetter = static m => m?.ToString();
    ...
    private static object? ValueAspectGetter(object? rowObject)
    {
        if (rowObject == null || rowObject.GetType() != typeof(FindAndReplaceNode)) return null;
    ...
    private static string? PropertyAspectGetter(object? rowObject)
    {
        if (rowObject == null || rowObject.GetType() != typeof(FindAndReplaceNode)) return null;
    ...
    private void RefreshData()
    {
        ...
        if (rbStandard.Checked)
        {
            ...
            if (_showOnlyTypes.Count > 0)
                _items = _items.Where(i => _showOnlyTypes.Contains(i.Key.GetType()))
                    .ToDictionary(static i => i.Key, static i => i.Value);
    ...
    private Bitmap? GetImage(object model)
    {
        if (model is string)
    ...
    private string? GetHierarchy(object rowObject)
    {
        if (rowObject is not IMapsDirectlyToDatabaseTable m)
            return null;

        if (_items.TryGetValue(m, out var descendancy) != true) return null;

        return Backslashes().Replace(string.Join('\\', descendancy.GetUsefulParents()), "\\").Trim('\\');
    }

    private readonly Dictionary<RDMPCollection, Type[]> _startingEasyFilters
        = new()
        {
    ...
    private readonly Dictionary<Type, RDMPCollection> _easyFilterTypesAndAssociatedCollections = new()
    {
    ...
    {
        var types = _items.Keys.Select(static k => k.GetType()).Distinct().ToArray();
        Type[]? startingFilters = null;

        if (focusedCollection != RDMPCollection.None &&
            _startingEasyFilters.TryGetValue(focusedCollection, out var filter))
            startingFilters = filter;

        // if there are at least 2 Types of object let them filter
        if (types.Length > 1)
            foreach (var t in _easyFilterTypesAndAssociatedCollections.Keys)
            {
                ...
                b.BackgroundImage =
                    BackColorProvider.GetBackgroundImage(b.Size, _easyFilterTypesAndAssociatedCollections[t]);
                b.CheckedChanged += CollectionCheckedChanged;

                newFindToolStrip.Items.Add(b);
            }

        if (!UserSettings.AdvancedFindFilters) return;
        // these filters do nothing currently
        AddUserSettingCheckbox(static () => UserSettings.ShowInternalCatalogues, static v => UserSettings.ShowInternalCatalogues = v, "I", "Include Internal");
        AddUserSettingCheckbox(static () => UserSettings.ShowDeprecatedCatalogues, static v => UserSettings.ShowDeprecatedCatalogues = v, "D", "Include Deprecated");
        AddUserSettingCheckbox(static () => UserSettings.ShowColdStorageCatalogues, static v => UserSettings.ShowColdStorageCatalogues = v, "C", "Include Cold Storage");
        AddUserSettingCheckbox(static () => UserSettings.ShowProjectSpecificCatalogues, static v => UserSettings.ShowProjectSpecificCatalogues = v, "P", "Include Project Specific");
        AddUserSettingCheckbox(static () => UserSettings.ShowNonExtractableCatalogues, static v => UserSettings.ShowNonExtractableCatalogues = v, "E", "Include Extractable");
    }
    ...
    {
        if (sender is not ToolStripButton { Tag: Type togglingType } button) return;

        if (button.Checked)
            _showOnlyTypes.Add(togglingType);
        else
            _showOnlyTypes.Remove(togglingType);
