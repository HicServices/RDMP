...
public abstract class TableRepository : ITableRepository
{
    ...
    private void SetParameterToValue(DbParameter p, object propValue)
    {
        ...
        else if (propValue is string value && string.IsNullOrWhiteSpace(value))
            p.Value = DBNull.Value;
    ...
    public T[] GetAllObjectsWhere<T>(string property1, object value1, ExpressionType operand, string property2,object value2) where T : IMapsDirectlyToDatabaseTable
    {
        ...
        return GetAllObjectsWhere<T>(
            $"WHERE {property1}=@val1 {oper} {property2}=@val2", new Dictionary<string, object>()
            {
                { "@val1", value1 },
                { "@val2", value2 }
    ...
    public RevertableObjectReport HasLocalChanges(IMapsDirectlyToDatabaseTable localCopy)
    {
        ...
        foreach (var propertyInfo in GetPropertyInfos(localCopy.GetType()))
        {
            ...
            if (local is string s && string.IsNullOrWhiteSpace(s))
                local = null;

            if (db is string db1 && string.IsNullOrWhiteSpace(db1))
                db = null;
    ...
    /// a field which is named whatever your value in parameter columnWithObjectID is.  If you hate life you can pass a dbNullSubstitution (which must also be of type
    ...
    /// <param name="dbNullSubstitution"></param>
    /// <returns></returns>
    public IEnumerable<T> SelectAllWhere<T>(string selectQuery, string columnWithObjectID = null, Dictionary<string, object> parameters = null, T dbNullSubstitution = default) where T : IMapsDirectlyToDatabaseTable
    {
        columnWithObjectID ??= $"{typeof(T).Name}_ID";
        ...
        if(dbNullSubstitution != null)
            for (var i = 0; i < nullsFound; i++)
                toReturn.Add(dbNullSubstitution);
