...
public class UsefulStuff
{
    ...
    public static Regex RegexThingsThatAreNotNumbersOrLetters = new("[^0-9A-Za-z]+");
    public static Regex RegexThingsThatAreNotNumbersOrLettersOrUnderscores = new("[^0-9A-Za-z_]+");
        
    public static UsefulStuff GetInstance()
    {
        return _instance ??= new UsefulStuff();
    }

    public static bool IsBadName(string name)
    {
        return name?.Any(c=>Path.GetInvalidFileNameChars().Contains(c)) == true;
    }
    ...
    public bool CHIisOK(string sCHI)
    {
        ...
        if (long.TryParse(sCHI, out n) && sCHI.Length == 10)
        {
    ...
    public static FileInfo SprayFile(Assembly assembly, string manifestName, string file, string outputDirectory = null)
    {
        ...
        var fileToSpray = assembly.GetManifestResourceStream(manifestName);
