...
public class BoundDate : Bound
{
    ...
    public override ValidationFailure Validate(object value, object[] otherColumns, string[] otherColumnNames)
    {
        ...
        if (!IsWithinRange(d,otherColumns, otherColumnNames))
    ...
    private DateTime? SafeConvertToDate(object lookupFieldNamed)
    {
        ...
        switch (lookupFieldNamed)
        {
            case null:
                return null;
            case DateTime time:
                return time;
            case string named when string.IsNullOrWhiteSpace(named):
                return null;
            case string named:
                try
                {
                    return DateTime.Parse(named);
                }
