...
public class MEFChecker : ICheckable
{
    ...
    public void Check(ICheckNotifier notifier)
    {
        ...
        else
        {
            ...
            {
                notifier.OnCheckPerformed(new CheckEventArgs(
                    $"Could not find MEF class called {_classToFind}, we were looking for a suitable replacment (a Type with the same basic name) but we found {substitute.Length} subistutitions!!! ({substitute.Aggregate("", (s, n) => $"{s}{n.FullName},")}", CheckResult.Fail, null));
            }
        }
    }
}