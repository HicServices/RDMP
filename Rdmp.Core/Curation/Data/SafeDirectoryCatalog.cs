...
public class SafeDirectoryCatalog
{
    ...
    private void LoadDll(FileInfo f, ICheckNotifier listener)
    {
        ...
        {
            listener?.OnCheckPerformed(new CheckEventArgs($"Did not load '{f}' because it is not a dotnet assembly",
                CheckResult.Success));
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    ///     Returns true if the <paramref name="newOneVersion" /> is a later version than
    ///     <paramref name="existingOneVersion" />.
    ...
    private void AddTypes(FileInfo f, Type[] types, ICheckNotifier listener)
    {
        ...
        listener?.OnCheckPerformed(new CheckEventArgs($"Successfully loaded Assembly {f.FullName} into memory",
            CheckResult.Success));
    }
