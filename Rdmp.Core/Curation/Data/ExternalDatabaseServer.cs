    ...
    ICheckable
{
    ...
    /// <summary>
    ///     If the database was created by an RDMP schema (or plugin schema) this will contain the name of the dll which holds
    ///     the schema e.g. DataQualityEngine.Database and was
    ...
    /// <summary>
    ///     The public network share of the Data path where the physical database files are stored if applicable.  Sharing your
    ///     database directory on the network is a
    ///     terrible idea (don't do it).  You can use this to automate detatching and shipping an MDF to your researchers e.g.
    ///     MsSqlReleaseSource
    /// </summary>
    ...
    /// <summary>
    ///     Creates a new persistent server reference in RDMP platform database that allows it to connect to a (usually
    ///     database) server.
    ///     <para>
    ///         If you are trying to create a database (e.g. a logging database) you should instead use
    ///         <see cref="MapsDirectlyToDatabaseTable.Versioning.MasterDatabaseScriptExecutor" />
    ///     </para>
    /// </summary>
    /// <param name="repository"></param>
    /// <param name="name"></param>
    /// <param name="creatorIfAny">
    ///     If the database referenced was created according to a specific SQL schema, this is the
    ///     schema provider
    /// </param>
    ...
    /// <summary>
    ///     Sets server,database,username and password properties based on the supplied DiscoveredDatabase (which doesn't have
    ///     to actually exist).  This method also optionally calls
    ...
    {
        return !string.IsNullOrWhiteSpace(CreatedByAssembly) &&
