...
/// <summary>
///     Record of a single component extracted as part of an <see cref="IExtractionConfiguration" />.  This could be an
///     anonymised dataset or bundled supporting
///     documents e.g. Lookups , pdfs etc.  This audit is used to perform release process (where all extracted artifacts
///     are collected and sent somewhere).
/// </summary>
public interface IExtractionResults : IReferenceOtherObject, IMapsDirectlyToDatabaseTable, ISaveable
{
    /// <summary>
    ///     Description of the file path, database table name etc of the extracted artifact.  This must make sense to the
    ///     pipeline
    ...
    /// <summary>
    ///     Type of the destination (final) component in the <see cref="IPipeline" /> used for extraction.  This is required so
    ///     that
    ///     artifacts can be collected again e.g. for release (Releasing from a flat file destination is different from
    ///     releasing
    ...
    /// <summary>
    ///     Null if the extraction completed succesfully.  Otherwise populated with the fatal error that caused the extraction
    ///     to stop.
    ...
    /// <summary>
    ///     The SQL Query used to extract records.  It is important that this is accurate since it is used to detect
    ///     configuration changes
    ///     (e.g. when trying to release a dataset in which the extracted artifacts are stale due to configuration changes in
    ///     the live system vs
    ///     the audit).
    /// </summary>
    ...
    ///     Type of the destination (final) component in the <see cref="IPipeline" /> used for
    ///     extraction
    /// </param>
