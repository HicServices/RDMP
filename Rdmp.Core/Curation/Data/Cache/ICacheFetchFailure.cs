...
public interface ICacheFetchFailure : ISaveable, IDeleteable, IMapsDirectlyToDatabaseTable
{
    ...
    /// <summary>
    ///     The time in 'dataset time' for which the request errored.  For example if the cache fetch request was for 10:00am -
    ///     11:00am on 2001-01-01 then the <see cref="FetchRequestStart" />
    ///     would be 10:00 2001-01-01 and the <see cref="FetchRequestEnd" /> would be 11:00 2001-01-01.  This has no bearing on
    ///     the time the process was running at or the time
    ...
    /// <summary>
    ///     The Exception resulted from the cache fetch request which documents what went wrong (e.g. 404 file not found,
    ...
    /// <summary>
    ///     The date at which we were able to make a succesful request for the time period defined by
    ///     <see cref="FetchRequestStart" /> and <see cref="FetchRequestEnd" />.  If this
    ///     date is populated then it means that although we were unable to fetch the period when we first requested it we were
    ///     subsequently able to rerun that period and the
