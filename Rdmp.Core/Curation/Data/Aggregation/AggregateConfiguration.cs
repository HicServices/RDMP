...
/// <summary>
///     Entry point for the aggregation system.  This class describes what a given aggregation is supposed to achieve (e.g.
///     summarise the number of records in a
///     dataset by region over time since 2001 to present).  An AggregateConfiguration belongs to a given Catalogue and is
///     the hanging-off point for the rest of
///     the configuration (e.g. AggregateDimension / AggregateFilter)
///     <para>
///         AggregateConfigurations can be used with an AggregateBuilder to produce runnable SQL which will return a
///         DataTable containing results appropriate to the
///         query being built.
///     </para>
///     <para>There are Three types of AggregateConfiguration (these are configurations - not separate classes):</para>
///     <para>
///         1. 'Aggregate Graph' - Produce summary information about a dataset designed to be displayed in a graph e.g.
///         number of records each year by healthboard
///     </para>
///     <para>
///         2. 'Cohort Aggregate' - Produce a list of unique patient identifiers from a dataset (e.g. 'all patients with
///         HBA1c test code > 50 in biochemistry')
///     </para>
///     <para>
///         3. 'Joinable PatientIndex Table' - Produce a patient identifier fact table for joining to other Cohort
///         Aggregates during cohort building (See JoinableCohortAggregateConfiguration)
///     </para>
///     <para>
///         The above labels are informal terms.  Use IsCohortIdentificationAggregate and IsJoinablePatientIndexTable to
///         determine what type a given
///         AggregateConfiguration is.
///     </para>
///     <para>
///         If your Aggregate is part of cohort identification (Identifier List or Patient Index Table) then its name will
///         start with cic_X_ where X is the ID of the cohort identification
    ...
    IDisableable, IKnowWhatIAm, IMightBeReadOnly, IRootFilterContainerHost
{
    ...
    /// <summary>
    ///     The ID of the Catalogue (dataset) that the AggregateConfiguration belongs to.  This determines which tables/server
    ///     it will be run on in addition to what filters/columns are
    ///     importable etc.
    /// </summary>
    ...
    /// <summary>
    ///     Automatically populated field indicating when the AggregateConfiguration was created in the database (you really
    ///     shouldn't change this field)
    /// </summary>
    ...
    /// <summary>
    ///     Indicates the AggregateDimension (if any) that will result in a pivot graph being generated.  E.g. if your
    ///     AggregateConfiguration is a graph of records by year between
    ///     2001 and 2018 then specifying a pivot on healthboard would result in 1 line in the graph per healthboard instead of
    ///     a single line for the count of all (the default).
    ...
    /// <summary>
    ///     Flag that indicates whether an AggregateConfiguration which is functioning as a graph can be exposed to users
    ///     without worrying about governance.  This manifests as whether
    ...
    /// <summary>
    ///     Specifies that HAVING section of the GROUP BY statement represented by this AggregateConfiguration.  For example
    ///     you could specify patients on drug X HAVING count(*) > 2 to
    ///     indicate that they must have had 2+ of the drug (ever or in the month being looked at if it is a graph).  This can
    ///     have unexpected consequences if you have a pivot and axis
    ///     etc since the having will apply only to the specific bucket (date section and pivot value) being evaluated at each
    ///     step.
    /// </summary>
    ...
    /// <summary>
    ///     ID of the AND/OR container of filters (which might be empty) that will restrict the records matched by the
    ///     AggregateConfiguration GROUP by.  All filters/containers will
    ...
    /// <summary>
    ///     Specify instead of RootFilterContainer_ID to indicate that this AggregateConfiguration should instead use the
    ///     filters of a different AggregateConfiguration.  This is
    ///     generally only useful if you have an AggregateConfiguration which you are using in cohort generation (e.g.
    ///     prescriptions for drug x) and you want to generate another
    ///     AggregateConfiguration which is a graph of those results by year and you don't want to duplicate the filter
    ///     configuration.
    /// </summary>
    ...
    /// <summary>
    ///     An AggregateConfiguration is a Group By statement.  This will involve using at least one Table in the FROM section
    ///     of the query.  The descision on which tables
    ///     to join is made by the AggregateBuilder based on the AggregateDimensions (columns).  If there are no column mapped
    ///     AggregateDimensions (e.g. there is only a count(*))
    ///     or there are other tables you want joined in addition the user can specify them in this property Populated via
    ///     <see cref="AggregateForcedJoin" />
    /// </summary>
    [NoMappingToDatabase]
    public ITableInfo[] ForcedJoins => CatalogueRepository.AggregateForcedJoinManager.GetAllForcedJoinsFor(this);

    /// <summary>
    ///     When an AggregateConfiguration is used in a cohort identification capacity it can have one or more 'patient index
    ///     tables' defined e.g.
    ///     'Give me all prescriptions for morphine' (Prescribing) 'within 6 months of patient being discharged from hospital'
    ///     (SMR01).  In this case
    ///     a join is done against the secondary dataset.
    ///     <para>
    ///         This property returns all such 'patient index table' AggregateConfigurations which are currently being used by
    ...
    /// <summary>
    ///     Only populated if the AggregateConfiguration is acting as a patient index table.  Returns the
    ///     <see cref="JoinableCohortAggregateConfiguration" /> object
    ...
    /// <summary>
    ///     An AggregateConfiguration is a Group By statement.  This will return all the SELECT columns for the query
    ///     (including any count(*) / sum(*) etc columns).
    /// </summary>
    [NoMappingToDatabase]
    ...
    ///     <see cref="IsCohortIdentificationAggregate" />).
    ///     <para>
    ///         The order location of an AggregateConfiguration within its parent <see cref="CohortAggregateContainer" /> (if
    ///         it has one).  This is mostly irrelevant for UNION /
    ///         INTERSECT operations (other than helping the user viewing the system) but is vital for EXCEPT containers where
    ///         the first AggregateConfiguration in the container is
    ...
    /// <summary>
    ///     True if the AggregateConfiguration is part of a <see cref="CohortIdentificationConfiguration" /> and is intended to
    ///     produce list of patient identifiers (optionally
    ...
    /// <summary>
    ///     All AggregateConfigurations have the potential a'Joinable Patient Index Table' (see AggregateConfiguration class
    ///     documentation).  This method injects
    ///     what fact that the AggregateConfiguration is definetly one by passing the JoinableCohortAggregateConfiguration that
    ///     makes it one.  Pass null in to
    ///     indicate that the AggregateConfiguration is definetly NOT ONE.  See also the method
    ///     <see cref="IsJoinablePatientIndexTable" />
    /// </summary>
    ...
    /// <summary>
    ///     Returns the Name.  If the AggregateConfiguration is a cohort identification aggregate (distinguished by
    ///     <see cref="CohortIdentificationConfiguration.CICPrefix" />)
    ...
    /// <summary>
    ///     Gets an IQuerySyntaxHelper from the Catalogue powering this AggregateConfiguration (
    ///     <see cref="IHasQuerySyntaxHelper.GetQuerySyntaxHelper" />)
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Specifies that that given <see cref="ExtractionInformation" /> should become a new SELECT column in the GROUP BY of
    ///     this AggregateConfiguration.  This
    ///     column will also appear in the ORDER BY and GROUP BY sections of the query when built by
    ///     <see cref="AggregateBuilder" />.  Finally if the column comes
    ///     from a novel underlying TableInfo then that new table will also be included in the FROM section of the query (e.g.
    ///     with a join).
    /// </summary>
    ...
    /// <summary>
    ///     Sets up a new <see cref="AggregateBuilder" /> with all the columns (See <see cref="AggregateDimensions" />), WHERE
    ///     logic (See <see cref="RootFilterContainer" />, Pivot
    ///     etc.
    /// </summary>
    /// <remarks>
    ///     Note that some elements e.g. axis are automatically handles at query generation time and therefore do not have
    ///     to be injected into the <see cref="AggregateBuilder" />
    /// </remarks>
    /// <param name="topX">
    ///     Maximum number of rows to return, the proper way to do this is via <see cref="AggregateTopX" />
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     If an AggregateConfiguration is set up as a graph (i.e. it isn't a cohort identification set) then it can have a
    ///     single <see cref="AggregateDimension" /> defined
    ///     as an axis dimension (e.g. DatePrescribed).  This method returns the  <see cref="AggregateContinuousDateAxis" /> if
    ///     there is one or null if there isn't one.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Returns the AggregateTopX defined for limiting the number of rows returned by the AggregateConfiguration or null if
    ///     there isn't one defined.  This is only applicable
    ...
    /// <summary>
    ///     Determines whether the AggregateConfiguration could be used in a <see cref="CohortIdentificationConfiguration" />
    ///     to identify a list of patients.  This will be true
    ///     if there are no pivot/axis dimensions and one of the <see cref="AggregateDimensions" /> is marked
    ///     <see cref="AggregateDimension.IsExtractionIdentifier" />
    /// </summary>
    ...
    /// <summary>
    ///     All AggregateConfigurations have the potential a'Joinable Patient Index Table' (see AggregateConfiguration class
    ///     documentation).  This method returns
    ///     true if there is an associated JoinableCohortAggregateConfiguration that would make an ordinary
    ///     AggregateConfiguration into a 'Patient Index Table'.
    /// </summary>
    /// <returns>
    ///     true if the AggregateConfiguration is part of a cic fulfilling the role of 'Patient Index Table' as defined by the
    ///     existence of a
    ///     JoinableCohortAggregateConfiguration object
    /// </returns>
    ...
    /// <summary>
    ///     If the AggregateConfiguration is set up as a cohort identification set or patient index table then this method will
    ...
    /// <summary>
    ///     Creates a complete clone copy of the current AggregateConfiguration.  The clone will include all new
    ///     AggregateDimensions, IFilters, IContainers etc.
    ///     IMPORTANT: This method is designed for cohort identifying AggregateConfigurations only and therefore does not
    ///     support Axis / TopX / Pivot.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Using the set; on Order property changes the Order. if you want to informt his object of its Order because you
    ///     already know what it is but this object doesn't know yet
    ...
    /// <summary>
    ///     Deletes the AggregateConfiguration.  This includes removing it from its <see cref="CohortAggregateContainer" /> if
    ///     it is part of one.  Also includes deleting its
    ///     <see cref="JoinableCohortAggregateConfiguration" /> if it is a 'patient index table'.
    /// </summary>
    /// <exception cref="NotSupportedException">
    ///     Thrown if the AggregateConfiguration is a patient index table that is being
    ///     used by other AggregateConfigurations
    /// </exception>
    ...
    /// <summary>
    ///     Returns a description of the role of the <see cref="AggregateConfiguration" /> based on whether it is set up as a
    ///     cohort, patient index table or cohort identification aggregate.
    /// </summary>
    /// <returns></returns>
