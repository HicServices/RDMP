    ...
    IInjectKnown<CatalogueExtractabilityStatus>
{
    ...
    /// <summary>
    ///     Use to set LoadMetadata to null without first performing Disassociation checks.  This should only be used for
    ///     in-memory operations such as cloning
    ///     This (if saved to the original database it was read from) could create orphans - load stages that relate to the
    ///     disassociated catalogue.  But if
    ///     you are cloning a catalogue and dropping the LoadMetadata then you won't be saving the dropped state to the
    ///     original database ( you will be saving it
    ...
    {
        return IsAcceptableName(name, out var whoCares);
    }
