...
using Rdmp.Core.Curation.Data.Datasets.Jira.API;
...
namespace Rdmp.Core.Curation.Data.Datasets.Jira
{
    ...
    public class JiraDatasetProvider : PluginDatasetProvider
    {
        ...
        public JiraDatasetProvider(IBasicActivateItems activator, DatasetProviderConfiguration configuration, HttpClient httpClient = null) : base(activator, configuration)
        {
            ...
            var code = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{credentials.Username}:{apiKey}"));
        ...
        public override JiraDataset Create(Catalogue catalogue)
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                List<JiraDatasetObjects.ObjectType> objectSchemaTypes = JsonConvert.DeserializeObject<List<JiraDatasetObjects.ObjectType>>(detailsString);
                ...
                var o = new CreateAtrObj()
                {
                    ...
                    Attributes = new List<JiraDatasetObjects.Attribute>() { new JiraDatasetObjects.Attribute() {
                objectTypeAttributeId=nameAttributeId,
                objectAttributeValues = new List<JiraDatasetObjects.ObjectAttributeValue>(){
        ...
        public override void Update(string uuid, PluginDataset datasetUpdates)
        {
            ...
            {
                var obj = new JiraDatasetObjects.Attribute();
                obj.objectTypeAttributeId = attribute.objectTypeAttributeId;
                obj.objectAttributeValues = new List<JiraDatasetObjects.ObjectAttributeValue>();
                foreach (var v in attribute.objectAttributeValues)
                {
                    obj.objectAttributeValues.Add(new JiraDatasetObjects.ObjectAttributeValue()
                    {
                        value = v.value
                    });
                }
        ...
        private List<JiraDatasetObjects.ReferencedObject> _schemaObjects = [];

        private List<JiraDatasetObjects.ReferencedObject> GetSchemaAttributes(string objectSchemaID)
        {
            ...
            if (otresponse.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (o is not null)
                {
                    ...
                    {
                        var detailsString = Task.Run(async () => await response.Content.ReadAsStringAsync()).Result;
                        _schemaObjects = JsonConvert.DeserializeObject<List<JiraDatasetObjects.ReferencedObject>>(detailsString);
        ...
        private JiraDatasetObjects.Attribute GenerateUpdateAttribute(JiraDataset dataset, Catalogue catalogue, string name, string value)
        {
            ...
            return new JiraDatasetObjects.Attribute()
            {
                objectTypeAttributeId = otai,
                objectAttributeValues = new List<JiraDatasetObjects.ObjectAttributeValue>() {
                new JiraDatasetObjects.ObjectAttributeValue()
                {
                    value = value
                }
            }
        ...
        public override void UpdateUsingCatalogue(Dataset dataset, Catalogue catalogue)
        {
            ...
            GenerateUpdateAttribute(jiraDataset, catalogue, "Short Description", catalogue.ShortDescription),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Acronym", catalogue.Acronym),
            GenerateUpdateAttribute(jiraDataset, catalogue, "DOI", catalogue.Doi),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Update Frequency", ((Catalogue.UpdateFrequencies)catalogue.Update_freq).ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Initial Release Date", catalogue.DatasetReleaseDate.ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Update Lag", ((Catalogue.UpdateLagTimes)catalogue.UpdateLag).ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Is Deprecated", catalogue.IsDeprecated.ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "Is Project Specific", catalogue.IsProjectSpecific(Activator.RepositoryLocator.DataExportRepository).ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "RDMP_CatalogueID", catalogue.ID.ToString()),
            GenerateUpdateAttribute(jiraDataset, catalogue, "RDMP_CatalogueDB", (catalogue.CatalogueRepository as TableRepository).GetConnection().Connection.ConnectionString),
            };
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                var detailsString = Task.Run(async () => await response.Content.ReadAsStringAsync()).Result;
                AQLResult databases = JsonConvert.DeserializeObject<AQLResult>(detailsString);

                var dbUpdate = new JiraDatasetObjects.Attribute();
                dbUpdate.objectId = jiraDataset.id;
                dbUpdate.objectTypeAttributeId = databaseTableschema;
                dbUpdate.objectAttributeValues = databases.values.Select(d => new JiraDatasetObjects.ObjectAttributeValue() { value = d.objectKey }).ToList();


                var dbs = new List<Value>();
                ...
                updateDataset.attributes.Add(new JiraDatasetObjects.Attribute()
                {
                    objectTypeAttributeId = databaseTableschema,
                    objectAttributeValues = dbs.Select(v => new JiraDatasetObjects.ObjectAttributeValue() { value = v.objectKey }).ToList()
                });
            }
            else
            {
