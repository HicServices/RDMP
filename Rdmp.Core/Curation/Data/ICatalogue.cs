...
/// <summary>
///     The central class for the RDMP, a Catalogue is a virtual dataset e.g. 'Hospital Admissions'.  A Catalogue can be a
///     merging of multiple underlying tables and exists
///     independent of where the data is actually stored (look at other classes like TableInfo to see the actual locations
///     of data).
///     <para>
///         As well as storing human readable names/descriptions of what is in the dataset it is the hanging off point for
///         Attachments (SupportingDocument), validation logic,
///         extractable columns (CatalogueItem->ExtractionInformation->ColumnInfo) ways of filtering the data, aggregations
///         to help understand the dataset etc.
///     </para>
    ...
    ICheckable, IHasFolder
{
    ...
    /// <summary>
    ///     The load configuration (if any) which is used to load data into the Catalogue tables.  A single
    ///     <see cref="LoadMetadata" /> can load multiple Catalogues.
    /// </summary>
    ...
    /// <summary>
    ///     Currently configured validation rules for columns in a Catalogue, this can be deserialized into a
    ///     Rdmp.Core.Validation.Validator
    /// </summary>
    string ValidatorXML { get; set; }

    /// <summary>
    ///     The <see cref="ExtractionInformation" /> which indicates the time field (in dataset time) of the dataset.  This
    ///     should be a column in your table
    ///     that indicates for every row when it became active e.g. 'PrescribedDate' for prescribing.  Try to avoid using
    ///     columns that have lots of nulls or
    ///     where the date is arbitrary (e.g. 'RecordLoadedDate')
    /// </summary>
    int? TimeCoverage_ExtractionInformation_ID { get; set; }

    /// <summary>
    ///     The <see cref="ExtractionInformation" /> which can provide a useful subdivision of the dataset e.g. 'Healthboard'.
    ///     This should be a logical subdivision
    ///     that helps in the assesment of data quality e.g. you might imagine that if you have 10% errors in data quality and
    ///     10 healthboards knowing that all the errors
    ...
    /// <summary>
    ///     Bit flag indicating whether the Catalogue is a seldom used dataset that should be hidden by default.  Use this if
    ///     you are importing lots of researcher
    ...
    /// <summary>
    ///     User specified period on how regularly the dataset is updated.  This does not have any technical bearing on how
    ///     often it is loaded
    ...
    /// <summary>
    ///     The alledged user specified date at which data began being collected.  For a more accurate answer you should run
    ///     the DQE (See also DatasetTimespanCalculator)
    ...
    /// <summary>
    ///     Returns all <see cref="AggregateConfiguration" /> that are associated with the Catalogue.  This includes both
    ///     summary graphs, patient index tables and all
    ...
    /// <summary>
    ///     Shorthand (recommended 3 characters or less) for referring to this dataset (e.g. 'DEM' for the dataset
    ///     'Demography')
    /// </summary>
    ...
    /// <summary>
    ///     Gets all distinct underlying <see cref="TableInfo" /> that are referenced by the <see cref="CatalogueItem" />s of
    ///     the Catalogue.  The tables are divided into
    ///     'normalTables' and 'lookupTables' depending on whether there are any <see cref="Lookup" /> declarations of
    ///     <see cref="LookupType.Description" /> on any of the
    ...
    /// <param name="lookupTables">
    ///     Unique TableInfos amongst all CatalogueItems in the Catalogue where there is at least
    ///     one <see cref="Lookup" /> declarations of <see cref="LookupType.Description" /> on the referencing ColumnInfo.
    /// </param>
    ...
    /// <summary>
    ///     Returns the unique <see cref="DiscoveredServer" /> from which to access connect to in order to run queries
    ///     generated from the <see cref="Catalogue" />.  This is
    ///     determined by comparing all the underlying <see cref="TableInfo" /> that power the
    ///     <see cref="ExtractionInformation" /> of the Catalogue and looking for a shared
    ///     servername.  This will handle when the tables are in different databases but only if you set
    ///     <paramref name="setInitialDatabase" /> to false
    /// </summary>
    /// <param name="context"></param>
    /// <param name="setInitialDatabase">
    ///     True to require all tables be in the same database.  False will just connect to master
    ...
    /// <summary>
    ///     Returns all <see cref="TableInfo" /> that underly the <see cref="Catalogue" />.  Returned array does not include
    ///     lookup tables unless the
    ...
    /// <summary>
    ///     Returns all <see cref="ExtractionInformation" /> declared under this <see cref="Catalogue" />
    ///     <see cref="CatalogueItem" />s.  This can be restricted by
    ///     <see cref="ExtractionCategory" />
    ///     <para>
    ///         pass <see cref="ExtractionCategory.Any" /> to fetch all <see cref="ExtractionInformation" /> regardless of
    ///         category
    ///     </para>
    /// </summary>
    ...
    /// <summary>
    ///     Overload for <see cref="GetAllExtractionInformation(ExtractionCategory)" /> using
    ///     <see cref="ExtractionCategory.Any" />
    /// </summary>
    /// <returns></returns>
    ...
    ///     where the  <see cref="IFilter.IsMandatory" /> flag is set.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Returns the unique <see cref="DatabaseType" /> shared by all <see cref="TableInfo" /> which underlie the Catalogue.
    ///     This is similar to GetDistinctLiveDatabaseServer
    ///     but is faster and more tolerant of failure i.e. if there are no underlying <see cref="TableInfo" /> at all or they
    ///     are on different servers this will still return
    ...
    /// <summary>
    ///     Returns the extractability of the Catalogue if it is known.  If it is not known then the repository will be used to
    ///     find out (and the result will be cached)
    ...
    /// <summary>
    ///     Provides a new instance of the object (in the database).  Properties will be copied from this object (child objects
    ///     will not be created).
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Returns true if the Catalogue is extractable but only with a specific Project.  You can pass null if you are
    ///     addressing a Catalogue for whom you know
