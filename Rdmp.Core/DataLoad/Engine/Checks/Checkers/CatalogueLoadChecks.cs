...
internal class CatalogueLoadChecks:ICheckable
{
    ...
    private void CheckTableInfoSynchronization(TableInfo tableInfo, ICheckNotifier notifier)
    {
        ...
        else
        {
            ...
            if (launchSyncFixer)
            {
                ...
                if(!userFixed)
                    notifier.OnCheckPerformed(new CheckEventArgs(
                        $"TableInfo {tableInfo} still failed Synchronization check", CheckResult.Fail, null)); //passed synchronization
            }
        }
    }
    ...
    private void CheckTableHasColumnInfosAndPrimaryKeys(DiscoveredDatabase live, TableInfo tableInfo, out ColumnInfo[] columnInfos, out ColumnInfo[] columnInfosWhichArePrimaryKeys, ICheckNotifier notifier)
    {
        ...
        if(primaryKeys.Any(k=>k.IsAutoIncrement))
            notifier.OnCheckPerformed(new CheckEventArgs(
                $"AutoIncrement columns {string.Join(",", primaryKeys.Where(k => k.IsAutoIncrement))} are not allowed as Primary Keys for your table because there is no way to differentiate new data being loaded from duplicate old data being loaded (the entire purpose of the RDMP DLE)",CheckResult.Fail));
        ...
        if (actualPks.Length != pksWeExpect.Length)
            notifier.OnCheckPerformed(new CheckEventArgs(
                $"Primary keys in Catalogue for database table {tableInfo.GetRuntimeName()} does not match Catalogue entry (difference in number of keys)", CheckResult.Fail, null));
        else
        ...
        if (requireSameNumberAndOrder)
        {
            ...
            if (stagingCols.Length != liveCols.Length)
            {
                ...
                for (var i = 0; i < stagingCols.Length; i++)
                    ...
                    {
                        notifier.OnCheckPerformed(new CheckEventArgs(
                            $"Column name/order mismatch between staging and live in table {tableName}, column {i} is {stagingCols[i]} in staging but is {liveCols[i]} in live.", CheckResult.Fail, null));
                        passedColumnOrderCheck = false;
                        break;
                    }
            ...
            if (passedColumnOrderCheck)
                notifier.OnCheckPerformed(new CheckEventArgs(
                    $"Column order match confirmed between staging and live on table {tableName}", CheckResult.Success, null));
        }
    }
    ...
    private void ConfirmNullability(DiscoveredColumn column, bool expectedNullability, ICheckNotifier notifier)
    {
        ...
        notifier.OnCheckPerformed(new CheckEventArgs(
            $"Nullability of {column} is AllowNulls={nullability}, (expected {expectedNullability})", nullability == expectedNullability ? CheckResult.Success : CheckResult.Fail, null));
    }
    #endregion
}