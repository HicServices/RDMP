...
FROM {$"[{dbInfo.GetRuntimeName()}]..[{tableInfo.GetRuntimeName()}]"} {tableAlias}";
        ...
        for (var i = 0; i < joinPath.Count; i++)
        {
            ...
            if (ascending)
            {
                ...
                sql += $@"
LEFT JOIN {$"[{dbInfo.GetRuntimeName()}]..[{parentTable.GetRuntimeName()}]"} {parentTableAlias} ON {parentTableAlias}.{join.PrimaryKey.GetRuntimeName()} = {childTableAlias}.{join.ForeignKey.GetRuntimeName()}";
            }
            else
            {
                ...
                sql += $@"
LEFT JOIN {$"[{dbInfo.GetRuntimeName()}]..[{childTable.GetRuntimeName()}]"} {childTableAlias} ON {parentTableAlias}.{join.PrimaryKey.GetRuntimeName()} = {childTableAlias}.{join.ForeignKey.GetRuntimeName()}";
            }
