...
public class WebFileDownloader : IPluginDataProvider
{
    ...
    {
        t.Start();
        var request = CreateNewRequest(UriToFile.AbsoluteUri);
            
        var writer = File.CreateText(destinationFile.FullName);
        ...
        catch (WebException e)
        {
            ...
            if (e.Response.Headers.AllKeys.Any(h => h.Contains("WWW-Authenticate")))
                job.OnNotify(this,
                    new NotifyEventArgs(ProgressEventType.Warning,
                        $"received {e.Status} and a header containing WWW-Authenticate instructions to go to URL {e.Response.ResponseUri.AbsoluteUri}", null));
            else
            ...
            if(!e.Response.Headers["WWW-Authenticate"].Equals(websenseIs))
                throw new Exception(
                    $"Although we were challenged with a WWW-Authenticate header the content of the header was not '{websenseIs}' so we are not going to send it our websense credentials, the challenge header we got contained the following:'{e.Response.Headers["WWW-Authenticate"]}'");
