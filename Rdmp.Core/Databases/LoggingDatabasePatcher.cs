...
public sealed class LoggingDatabasePatcher : Patcher
{
    ...
    public override Patch GetInitialCreateScriptContents(DiscoveredDatabase db)
    {
        ...
        sql.AppendLine($"""
                        ...
                        /*create tasks*/
                        INSERT INTO {db.ExpectTable("DataLoadTask").GetWrappedName()} ({sh.EnsureWrapped("ID")}, {sh.EnsureWrapped("description")}, {sh.EnsureWrapped("name")}, {sh.EnsureWrapped("userAccount")}, {sh.EnsureWrapped("statusID")}, {sh.EnsureWrapped("isTest")}, {sh.EnsureWrapped("dataSetID")}) VALUES(1, 'Internal', 'Internal', 'Thomas', 1,  {(sh.DatabaseType == FAnsi.DatabaseType.PostgreSql ? false : 0)}, 'Internal');
                        INSERT INTO {db.ExpectTable("DataLoadTask").GetWrappedName()} ({sh.EnsureWrapped("ID")}, {sh.EnsureWrapped("description")}, {sh.EnsureWrapped("name")}, {sh.EnsureWrapped("userAccount")}, {sh.EnsureWrapped("statusID")}, {sh.EnsureWrapped("isTest")}, {sh.EnsureWrapped("dataSetID")}) VALUES(2, 'DataExtraction', 'DataExtraction', 'Thomas', 1,  {(sh.DatabaseType == FAnsi.DatabaseType.PostgreSql ? false : 0)}, 'DataExtraction');
