...
namespace Rdmp.Core.Databases
{
    public sealed class DataQualityEnginePatcher : Patcher
    {
        ...
        public override Patch GetInitialCreateScriptContents(DiscoveredDatabase db)
        {
            ...
            sql.AppendLine (db.Helper.GetCreateTableSql(db, "Evaluation", new[]
            {
            ...
            sql.AppendLine(db.Helper.GetCreateTableSql(db, "ColumnState", new[]
            {
                new DatabaseColumnRequest("ID",new DatabaseTypeRequest(typeof(int))){IsAutoIncrement = true, AllowNulls = false, IsPrimaryKey = true},
                columnState_Evaluation_ID = new DatabaseColumnRequest("Evaluation_ID",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("TargetProperty",new DatabaseTypeRequest(typeof(string),500)){AllowNulls = true},
                new DatabaseColumnRequest("DataLoadRunID",new DatabaseTypeRequest(typeof(int))){AllowNulls = true},
                new DatabaseColumnRequest("CountCorrect",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("CountDBNull",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("ItemValidatorXML",new DatabaseTypeRequest(typeof(string))){AllowNulls = true},
                new DatabaseColumnRequest("CountMissing",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("CountWrong",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("CountInvalidatesRow",new DatabaseTypeRequest(typeof(int))){AllowNulls = false},
                new DatabaseColumnRequest("PivotCategory",new DatabaseTypeRequest(typeof(string),50){Unicode = true}){AllowNulls = false},
