...
public sealed class DataQualityEnginePatcher : Patcher
{
    ...
    public override Patch GetInitialCreateScriptContents(DiscoveredDatabase db)
    {
        ...
        sql.AppendLine(
            $"{db.Helper.GetCreateTableSql(db, "Evaluation", new[] { new DatabaseColumnRequest("DateOfEvaluation", new DatabaseTypeRequest(typeof(DateTime))), new DatabaseColumnRequest("CatalogueID", new DatabaseTypeRequest(typeof(int))) { AllowNulls = false }, new DatabaseColumnRequest("ID", new DatabaseTypeRequest(typeof(int))) { IsAutoIncrement = true, IsPrimaryKey = true } }, null, false).TrimEnd()};");
