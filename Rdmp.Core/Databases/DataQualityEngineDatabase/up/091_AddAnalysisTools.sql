...
if not exists (select 1 from sys.tables where name = 'PrimaryConstraint')
BEGIN
CREATE TABLE [dbo].[PrimaryConstraint](
...
if not exists (select 1 from sys.tables where name = 'CatalogueValidationResultCounts')
BEGIN
...
if not exists (select 1 from sys.tables where name = 'FieldCompletionRate')
BEGIN
