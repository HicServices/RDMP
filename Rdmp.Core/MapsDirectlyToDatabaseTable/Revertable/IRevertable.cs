...
/// <summary>
///     Object (usually a IMapsDirectlyToDatabaseTable) which can have its state saved into a database but also have its
///     current state compared with the
...
public interface IRevertable : IMapsDirectlyToDatabaseTable, ISaveable
{
    ...
    /// <summary>
    ///     Connects to the database <see cref="IRepository" /> and checks the values of public properties against the
    ...
    /// <summary>
    ///     Connects to the database <see cref="IRepository" /> and returns true if the object (in memory) still exists in the
    ///     database.
    /// </summary>
    /// <returns></returns>
    bool Exists();
