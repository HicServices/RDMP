...
/// <summary>
///     Indicates that a class cannot exist in memory without simultaneously existing as a record in a database table.
///     This is how RDMP handles continuous access
///     by multiple users and persistence of objects as well as allowing for enforcing program logic via database
///     constraints.
///     <para>
///         RDMP basically treats the database as main memory and has many classes which are directly checked out, modified
///         and saved into the database.  These
///         classes must follow strict rules e.g. all public properties must directly match columns in the database table
///         holding them (See DatabaseEntity).  This is
...
{
    /// <summary>
    ///     Every database table that stores an <see cref="IMapsDirectlyToDatabaseTable" /> must have an identity column called
    ///     ID which must be the primary key.
    ...
    /// <summary>
    ///     Makes any persistent Property change attempts throw an Exception. (See also
