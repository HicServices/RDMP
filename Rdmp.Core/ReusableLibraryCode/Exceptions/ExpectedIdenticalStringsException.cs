...
public class ExpectedIdenticalStringsException : Exception
{
    ...
    private static string AssembleMessage(string message, string expected, string actual)
    {
        ...
        for (int i = 0; i < expected.Length; i++)
        {
            if (!(i < actual.Length))
                return message + Environment.NewLine +
                       "Strings are identical except that Actual string ends at character " + i +
                       " while we still Expected " + (expected.Length - actual.Length) + " additional characters";
        ...
        return message + Environment.NewLine +
               "Strings are identical except that Expected string ends at character " + expected.Length +
               " while the Actual string had " + (actual.Length - expected.Length) + " additional characters"
               + GetPreviewsAround(expected.Length,expected,actual);
    }
    ...
    private static string GetPreviewsAround(int i, string expected, string actual)
    {
        ...
        string differencePointer = "";

        for (int j = 0; j < iIsAtCharacterPosition + "EXPECTED:".Length; j++)
            differencePointer += "-";
    ...
    private static string GetPreviewAround(int i, string str, out int iIsAtCharacterPosition)
    {
        ...
        int startSubstringAt = Math.Max(0, i - charsBefore);
        ...
        int lengthWeWantToTake = (i - startSubstringAt) + charsAfter;//usually the full amount unless charsBefore is truncated due to the start of the string (when difference is early in the string)
        int lengthAvailable = str.Length - startSubstringAt;
        int lengthWeWillActuallyTake = Math.Min(lengthWeWantToTake, lengthAvailable);
