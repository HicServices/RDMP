    ...
    IAtomicCommandWithTarget
{
    ...
    public override void Execute()
    {
        ...
        {
            _cic = (CohortIdentificationConfiguration)_activator.SelectOne("Select a Cohort Identification Configuration", _activator.RepositoryLocator.CatalogueDbContext.GetAllObjects<CohortIdentificationConfiguration>().ToArray());
            if (_cic is null) return;
        }
        if (_cic is not null && _project is null && _associationToDelete is null)
        {

            _existingAssociations = _activator.RepositoryLocator.CatalogueDbContext.GetAllObjectsWhere<ProjectCohortIdentificationConfigurationAssociation>("CohortIdentificationConfiguration_ID", _cic.ID).ToArray();
        }
        if (_cic is null && _project is not null && _associationToDelete is null)
        {

            _existingAssociations = _activator.RepositoryLocator.DataExportRepository.GetAllObjectsWhere<ProjectCohortIdentificationConfigurationAssociation>("Project_ID", _project.ID).ToArray();
        }
