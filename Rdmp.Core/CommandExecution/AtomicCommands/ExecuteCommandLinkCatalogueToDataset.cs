...
public sealed class ExecuteCommandLinkCatalogueToDataset : BasicCommandExecution
{
    ...
    public ExecuteCommandLinkCatalogueToDataset(IBasicActivateItems activator, [DemandsInitialization("The catalogue To link")]Catalogue catalogue, [DemandsInitialization("The dataset to link to")] EntityFramework.Models.Dataset dataset, bool linkAllOtherColumns = true) : base(activator)
    {
    ...
    public override void Execute()
    {
        ...
        foreach (var ci in items.Select(static item => item.ColumnInfo).Where(ci => ci?.Dataset.ID != _dataset.ID))
        {
            ...
            var catalogueItems = ci.CatalogueDbContext.GetAllObjects<EntityFramework.Models.ColumnInfo>().Where(ci => ci.Name[..ci.Name.LastIndexOf(".", StringComparison.Ordinal)] == databaseName).ToList();
            foreach (var aci in catalogueItems)
            {
                aci.Dataset = _dataset;
            }
            ci.CatalogueDbContext.SaveChanges();
