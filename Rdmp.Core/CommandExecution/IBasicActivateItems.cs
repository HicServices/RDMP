...
{
    /// <summary>
    ///     True for activators that can elicit immediate responses from users.  False for activators designed to run
    ///     unattended e.g. command line/scripting
    /// </summary>
    ...
    /// <summary>
    ///     True to prompt Yes/No and offer common fixes for being unable to directly delete an object.  Note that
    ///     <see cref="IsInteractive" /> must be true for this flag to be respected
    /// </summary>
    ...
    /// <summary>
    ///     Returns the repository that stores objects of the given <paramref name="type" /> (Must be
    ...
    ///     If no repositories are found that own the given <paramref name="type" />
    /// </exception>
    IRepository GetRepositoryFor(Type type);

    /// <summary>
    ///     Component for recording object tree inheritance (for RDMPCollectionUI primarily but also for anyone who wants to
    ///     know children of objects or all objects quickly without having to go back to the database)
    /// </summary>
    ...
    /// <summary>
    ///     Component for telling you whether a given DatabaseEntity is one of the current users favourite objects and for
    ///     toggling it
    /// </summary>
    ...
    /// <summary>
    ///     Create a class capable of running a <see cref="IPipeline" /> under a given <see cref="IPipelineUseCase" />.  This
    ///     may be an async process e.g. non modal dialogues
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Prompts the user to enter a description for a cohort they are trying to create including whether it is intended to
    ///     replace an old version of another cohort.
    /// </summary>
    ...
    CohortHoldoutLookupRequest GetCohortHoldoutLookupRequest(ExternalCohortTable externalCohortTable, IProject project,
    ...
    /// <summary>
    ///     Prompts the user to pick from one of the <paramref name="availableObjects" /> one or more objects.  Returns null or
    ///     empty if
    ...
    /// <param name="initialSearchText"></param>
    /// <param name="allowAutoSelect">
    ///     Determines behaviour when <paramref name="available" /> has only one element.  True to
    ///     auto select the only object available
    /// </param>
    ...
    /// <summary>
    ///     Offers the user a binary choice returning the choice or false if user cancels (to distinguish between false and
    ///     cancel
    ///     use the overload).  This method is blocking.
    /// </summary>
    /// <param name="text">The question to pose</param>
    /// <param name="caption"></param>
    ...
    /// <summary>
    ///     Offers the user a binary choice for whether they want to proceede on a dangerous/complex operation.  Returns true
    ///     if
    ...
    /// <summary>
    ///     Display the given message to the user (e.g. in a MessageBox or out into the Console).  If provided
    ///     <paramref name="title" /> may also be featured in the presentation
    /// </summary>
    ...
    /// <summary>
    ///     Shows the given error message to the user, optionally with the <paramref name="exception" /> stack trace / Message
    ...
    /// <summary>
    ///     Block until the <paramref name="task" /> is completed with optionally showing the user some kind of ongoing
    ///     operation
    ...
    ///     This will only offer concrete classes and not abstract classes or interfaces.  To allow picking those use the
    ///     overload.
    /// </summary>
    /// <param name="prompt"></param>
    /// <param name="baseTypeIfAny">
    ///     Pass a base class or interface if the Type must be an inheritor / assignable to a specific
    ///     Type otherwise pass null
    /// </param>
    /// <param name="chosen"></param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Handle the creation and configuring of a new <see cref="ICatalogue" /> often with user input about what column(s)
    ///     should be extractable etc.  Return null if user cancelled the process somehow
    /// </summary>
    ...
    ///     Optional virtual folder into which to put the <see cref="ICatalogue" /> or null.  This is
    ///     not a file system folder only a visual display to the user in the RDMP client
    /// </param>
    /// <param name="extractionIdentifierColumns">
    ///     Optional, which column(s) should be
    ///     <see cref="ConcreteColumn.IsExtractionIdentifier" />
    /// </param>
    /// <returns>
    ///     A fully configured ready to go Catalogue or null if user cancelled process e.g. if
    ///     <see cref="IsInteractive" />
    /// </returns>
    ...
    /// <summary>
    ///     Returns true if creating a cohort through an interactive wizard is supported.  If a wizard was shown and suitable
    ///     input received then <paramref name="cic" /> should be populated with the result
    /// </summary>
    ...
    /// <summary>
    ///     Requests that the activator highlight or otherwise emphasise the supplied item.  Depending on who is subscribed to
    ///     this event nothing may actually happen
    /// </summary>
    ...
    /// <summary>
    ///     Prompts user or directly creates a new satelite database (e.g. logging / dqe etc) and returns a persistent
    ///     reference to it
    /// </summary>
    ...
    /// <param name="db">
    ///     The server in which the database should be created or null if the user is expected to pick themselves
    ///     as part of the method e.g. through a UI
    /// </param>
    ...
    /// <summary>
    ///     Component for auditing errors that should be brought to the users attention subtly (e.g. if a plugin crashes while
    ///     attempting to create menu items)
    /// </summary>
    ...
    /// <summary>
    ///     Called when <see cref="BasicCommandExecution.Publish" /> is invoked.  Allows you to respond to publish events
    ///     outside of UI code.  UI code
