    ...
    ICustomSearchString
{
    ...
    ///     the <see cref="DatabaseEntity.Repository" /> database using a transaction (to prevent a half successful clone being
    ///     generated).
    /// </summary>
    /// <returns></returns>
        ...
        using (repo.BeginNewTransaction())
        {
            try
            {
                ...
                foreach (SelectedDataSets selected in SelectedDataSets)
                {
                    ...
                    foreach (var cloneExtractableColumn in GetAllExtractableColumnsFor(selected.ExtractableDataSet)
                                 .Select(static extractableColumn => extractableColumn.ShallowClone()))
                    {
    ...
    private ExternalDatabaseServer GetDistinctLoggingServer(bool testLoggingServer)
    {
        ...
        foreach (int? catalogueID in GetAllExtractableDataSets().Select(ds => ds.Catalogue_ID))
        {
            ...
            _ = catalogue.LiveLoggingServer_ID ?? throw new Exception(
