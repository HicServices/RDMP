...
public class CohortDefinition : ICohortDefinition
{
    ...
    public bool IsAcceptableAsNewCohort(out string matchDescription)
    {
        ...
        {
            matchDescription =
                $"Error occured when checking for existing cohorts in the Project.  We were looking in {LocationOfCohort + Environment.NewLine + Environment.NewLine} Error was: {ExceptionHelper.ExceptionToListOfInnerMessages(ex)}";
