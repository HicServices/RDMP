...
/// <summary>
///     Represents the desire to extract a given dataset for a given <see cref="ExtractableDataSet" /> for a given
...
public interface ISelectedDataSets : IDeleteable, IRevertable, IMightBeReadOnly, IRootFilterContainerHost
{
    ...
    /// <summary>
    ///     <see cref="IExtractableDataSet" /> that has been selected for extraction in the
    ///     <see cref="ExtractionConfiguration_ID" />
    /// </summary>
    ...
    /// <summary>
    ///     Returns all tables which should be force joined against when extracting the <see cref="ISelectedDataSets" />
    ///     (regardless of extracted columns).
    ///     This does not include implicitly joined <see cref="ITableInfo" /> (i.e. if you are extracting a column from that
    ///     table).
    /// </summary>
    ISelectedDataSetsForcedJoin[] SelectedDataSetsForcedJoins { get; }

    /// <summary>
    ///     If this dataset has been extracted in the past this will return the last extract audit record.  Otherwise it will
    ///     return null
    /// </summary>
    /// <returns></returns>
