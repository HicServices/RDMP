...
public class ExtractableCohortAuditLogBuilder
{
    ...
    /// </summary>
    private readonly Regex _legacyCic = new(@"Created by running cic ([\d]+)");

    private readonly Regex _regexGetColumn = new(@$"{InColumn} '(.*)'");
    private readonly Regex _regexGetFilePath = new(@$"{InFile} '(.*)'");
    private readonly Regex _regexGetID = new(@"\(ID=(\d+)\)");
    ...
    /// <returns></returns>
    public object GetObjectIfAny(IExtractableCohort cohort, IRDMPPlatformRepositoryServiceLocator repositoryLocator)
    {
