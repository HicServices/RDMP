...
internal class AnalyticsRunner : Runner
{
    private readonly AnalyticsOptions _options;

    public AnalyticsRunner(AnalyticsOptions options)
    {
        _options = options;
    }
        ...
        ICheckNotifier checkNotifier, GracefulCancellationToken token)
    {
        ...
        var reporter = new AnalyticsReporter(catalogue);

        switch (_options.Command)
        {
            case CommandLineActivity.run:
                if(_options.DataLoad_ID != null)
                {
                    reporter.GenerateDataLoadAnalytics((int)_options.DataLoad_ID);
                }
