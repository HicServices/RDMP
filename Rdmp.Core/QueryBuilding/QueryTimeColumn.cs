...
public class QueryTimeColumn: IComparable
{
    ...
    public static void SetLookupStatus(QueryTimeColumn[] ColumnsInOrder, List<ITableInfo> tablesUsedInQuery)
    {
        ...
        foreach (var column in ColumnsInOrder)
        {
            ...
            var foreignKeyLookupInvolvement = allAvailableLookups.Where(l=>l.ForeignKey_ID == column.UnderlyingColumn.ID).ToArray();
            var lookupDescriptionInvolvement = allAvailableLookups.Where(l=>l.Description_ID == column.UnderlyingColumn.ID).ToArray();
            ...
            if (lookupDescriptionInvolvement.Length > 0)
            {
                ...
                column.LookupTableAlias = lookupTablesFound; // must belong to same one as previously encountered foreign key
