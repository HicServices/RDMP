...
public class SqlQueryBuilderHelper
{
    ...
    public static List<JoinInfo> FindRequiredJoins(ISqlQueryBuilder qb)
    {
        ...
        RDMPDbContext cataRepository;
        try
        {
            cataRepository = qb.TablesUsedInQuery.First().CatalogueDbContext;
        }
        catch (Exception e)
        {
            throw new Exception(
        ...
        foreach (TableInfo table1 in qb.TablesUsedInQuery)
            foreach (TableInfo table2 in qb.TablesUsedInQuery)
                if (table1.ID != table2.ID) //each table must join with a single other table
                {
                    ...
                    var availableJoins = new JoinManager(cataRepository).GetAllJoinInfosBetweenColumnInfoSets(
    ...
    /// <param name="qb"></param>
    /// <returns></returns>
    public static IEnumerable<EntityFramework.Models.Lookup> GetDistinctRequiredLookups(ISqlQueryBuilder qb) =>
        //from all columns
    ...
    private static List<ITableInfo> AddOpportunisticJoins(List<ITableInfo> toReturn, List<IFilter> filters)
    {
        ...
        {
            var available = new JoinManager(table.CatalogueDbContext).GetAllJoinInfosWhereTableContains(table, JoinInfoType.AnyKey);
