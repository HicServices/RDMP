...
public class ExtractionQueryBuilder
{
        ...
        IQuerySyntaxHelper syntaxHelper, List<string> ignoreBatchingForTheseIdentifiers)
    {
        ...
        var deltaWhere = $"{((ignoreBatchingForTheseIdentifiers is null || identifier == null || !ignoreBatchingForTheseIdentifiers.Any()) ? "" : $"OR {identifier.ColumnInfo.GetFullyQualifiedName()} in ({string.Join(',', ignoreBatchingForTheseIdentifiers.Select(i => $"'{i}'"))})")}";
        var line =
