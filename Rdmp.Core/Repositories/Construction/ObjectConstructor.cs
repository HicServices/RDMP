...
public class ObjectConstructor
{
    ...
    public static object ConstructIfPossible(Type typeToConstruct, params object[] constructorValues)
    {
        ...
        return compatible.Count != 0 ? InvokeBestConstructor(compatible, constructorValues) : null;
    }
    ...
    public static ConstructorInfo GetRepositoryConstructor(Type type)
    {
        var compatible = type.GetConstructors()
            ...
            .Where(static t => t.parameters.All(static p => p.GetType() != typeof(ShareManager) && p.GetType() != typeof(DbDataReader)))
            .Where(static t => t.parameters.Any(static p => typeof(IRepository).IsAssignableFrom(p.ParameterType)))
