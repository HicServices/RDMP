...
internal class TableInfoCredentialsManager : ITableInfoCredentialsManager
{
    ...
    /// <summary>
    /// Creates a new helper class instance for writing/deleting credential usages for <see cref="TableInfo"/> objects in the <paramref name="catalogueDbContext"/>
    /// </summary>
    /// <param name="catalogueDbContext"></param>
    public TableInfoCredentialsManager(RDMPDbContext catalogueDbContext)
    {
    ...
    public DataAccessCredentials GetCredentialsIfExistsFor(ITableInfo tableInfo, DataAccessContext context)
    {
        ...
        return toReturn != -1 ? _catalogueDbContext.GetObjectByID<DataAccessCredentials>(toReturn) : null;
    }
    ...
    {
        Dictionary<DataAccessContext, int> toReturn = new();
        ...
        return toReturn.ToDictionary(k => k.Key, v => _catalogueDbContext.GetObjectByID<DataAccessCredentials>(v.Value));
    }
        ...
        DataAccessCredentials credentials)
    {
        ...
        return toReturn.ToDictionary(k => k.Key,
            v => _catalogueDbContext.GetAllObjectsInIDList<TableInfo>(v.Value).Cast<ITableInfo>().ToList());
    }
    ...
    public DataAccessCredentials GetCredentialByUsernameAndPasswordIfExists(string username, string password)
    {
        ...
        var existingCredentials = _catalogueDbContext.GetAllObjects<DataAccessCredentials>()
