...
/// <summary>
///     Models the relationship between <see cref="TableInfo" /> and the <see cref="DataAccessCredentials" /> (if any) that
///     can be used to reach them.
/// </summary>
public interface ITableInfoCredentialsManager
{
    /// <summary>
    ///     Declares that the given <paramref name="tableInfo" /> can be accessed using the <paramref name="credentials" />
    ///     (username / encrypted password) under the
    ...
    /// <summary>
    ///     Removes the right to use passed <paramref name="credentials" /> to access the <paramref name="tableInfo" /> under
    ...
    ///     <para>
    ///         Tries to find a DataAccessCredentials for the supplied TableInfo.  For example you are trying to find a
    ///         username/pasword to use with the TableInfo when performing
    ///         a DataLoad, this method will first return any explicit usage allowances (if there is a credential liscenced for
    ///         use during DataLoad) if no such credentials exist
    ///         it will then check for a credential which is liscenced for Any usage (can be used for data load, data export
    ///         etc) and return that else it will return null
    ///     </para>
    /// </summary>
    /// <param name="tableInfo"></param>
    /// <param name="context"></param>
    ...
    /// <summary>
    ///     Fetches all <see cref="DataAccessCredentials" /> (username and encrypted password) that can be used to access the
    ///     <see cref="TableInfo" /> under any
    ...
    /// <summary>
    ///     Returns all credential usage permissions for the given set of <paramref name="allTableInfos" /> and
    ///     <paramref name="allCredentials" />
    /// </summary>
    ...
    /// <summary>
    ///     Returns the existing <see cref="DataAccessCredentials" /> if any which match the unencrypted
    ///     <paramref name="username" /> and <paramref name="password" /> combination.  Throws
