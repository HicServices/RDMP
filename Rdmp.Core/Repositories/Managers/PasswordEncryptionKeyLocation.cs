...
public class PasswordEncryptionKeyLocation : IEncryptionManager, IInjectKnown
{
    ...
    private RSAParameters? DeserializeFromLocation(string keyLocation)
    {
        ...
        try
        {
            var DeserializeXml = new XmlSerializer(typeof(RSAParameters));
            return (RSAParameters)DeserializeXml.Deserialize(new StringReader(xml));
        }
        catch (Exception e)
        {
            throw new Exception(
                $"Failed to deserialize key file {keyLocation}, possibly it is corrupt or has been modified manually to break it?", e);
        }
    }
    ...
    public FileInfo CreateNewKeyFile(string path)
    {
        ...
        var p = provider.ExportParameters(true);
        ...
        {
            var SerializeXml = new XmlSerializer(typeof(RSAParameters));
